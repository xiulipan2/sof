...
#include <sof/audio/crossover/crossover_helper.h>
...
static int multiband_drc_params(struct comp_dev *dev, struct sof_ipc_stream_params *params)
{
	...
	comp_info(dev, "multiband_drc_params()");
...
static int multiband_drc_prepare(struct comp_dev *dev)
{
	...
	/* Initialize DRC */
	comp_info(dev, "multiband_drc_prepare(), source_format=%d, sink_format=%d",
	...
	comp_info(dev, "multiband_drc_prepare(), DRC is configured.");
